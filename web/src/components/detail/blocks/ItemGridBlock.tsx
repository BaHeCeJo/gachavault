  ...
  if (!source || source === "related") {
    ...
    if (t === "backref") {
      cards = (relations.backRefs.get(source) ?? []).map((r) => ({
        id: r.id, name: r.name, image_url: r.image_url, href: itemHref(r),
      }));
    } else if (t === "itemref") {
      const v = data[source];
      const ref = typeof v === "string" ? relations.resolvedRefs.get(v) : undefined;
      cards = ref ? [{ id: ref.id, name: ref.name, image_url: ref.image_url, href: itemHref(ref) }] : [];
    } else {
      const v = data[source];
        ...
        {cards.map((card) => (
          ...
          >
            <div className="relative h-20">
              <SafeImage src={card.image_url} alt={card.name} fill sizes="(min-width: 768px) 140px, (min-width: 640px) 25vw, 33vw" className="object-cover group-hover:scale-105 transition-transform duration-200" fallback={
