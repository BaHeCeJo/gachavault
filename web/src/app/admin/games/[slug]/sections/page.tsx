...
export default function AdminGameSectionsPage() {
  ...
  const { user, isLoading } = useAdminGuard(`/admin/games/${slug}/sections`);
  ...
  const [initialForm, setInitialForm] = useState({ slug: "", name: "", order: "0", tabs: DEFAULT_TABS });
  ...
  const openCreate = () => {
    const fresh = { slug: "", name: "", order: "0", tabs: DEFAULT_TABS };
    setForm(fresh);
